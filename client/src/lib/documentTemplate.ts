...
export type DocumentTemplateInput = {
  ...
  signatureWorkflow?: {
    ...
    owner?: { status?: string | null; name?: string | null; reviewedAt?: string | Date | null } | null;
...
export function buildProfessionalDocumentHtml(identity: CompanyIdentity | undefined, input: DocumentTemplateInput) {
  ...
  const printScript = `${showFinancialSummary ? "" : "<script>document.querySelector('.summary')?.remove()</script>"}${input.autoPrint === false ? "" : "<script>window.onload=()=>setTimeout(()=>window.print(),250)</script>"}`;
