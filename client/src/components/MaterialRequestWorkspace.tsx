...
export function MaterialRequestWorkspace() {
  ...
  const { data: approvals = [] } = trpc.erp.approvals.list.useQuery();
  const [draft, setDraft] = useState(emptyDraft);
  const [editingId, setEditingId] = useState<number | null>(null);
  const [materialSearch, setMaterialSearch] = useState("");
  const [ownerCostSelections, setOwnerCostSelections] = useState<Record<number, string>>({});
  ...
  const decide = trpc.erp.procurement.requisitions.decide.useMutation({ onSuccess: () => { void utils.erp.procurement.requisitions.list.invalidate(); void utils.erp.approvals.list.invalidate(); } });

  const chooseMaterial = (materialId: string) => {
    setDraft({ ...draft, inventoryItemId: materialId, costItemId: "" });
  };
  const save = (event: React.FormEvent) => {
    event.preventDefault();
    if (!selectedProjectId || !draft.stageId || !selectedItemId || Number(draft.quantity) <= 0) return;
    const payload = { projectId: selectedProjectId, stageId: Number(draft.stageId), description: draft.description || undefined, requiredBy: draft.requiredBy || undefined, items: [{ inventoryItemId: selectedItemId, description: selectedMaterial?.name || "خامة", unit: selectedMaterial?.unit, quantity: Number(draft.quantity), estimatedUnitCost: Number(draft.estimatedUnitCost || 0) }] };
  ...
  return <div className="space-y-6" dir="rtl">
    <header className="rounded-2xl bg-[#18324b] p-6 text-white"><p className="text-sm text-[#f5d98c]">مراقبة طلبات المواد</p><h2 className="mt-1 text-2xl font-bold">طلب مادة مربوط بالخطة والمرحلة</h2><p className="mt-2 max-w-3xl text-sm text-white/75">اختر بطاقة الخامة والمرحلة وحدد الكمية والملاحظة. يختار المالك بند التكلفة بعد اعتماد مصطفى وقبل المراجعة النهائية لمدير المشاريع.</p></header>
    ...
    <Card className="border-0 shadow-sm"><CardHeader><CardTitle className="flex items-center gap-2 text-[#18324b]"><ClipboardList className="h-5 w-5" /> سجل طلبات المواد والرقابة</CardTitle></CardHeader><CardContent><div className="space-y-3">{requisitions.length === 0 ? <p className="rounded-xl bg-slate-50 p-6 text-center text-sm text-slate-500">لا توجد طلبات مواد مسجلة بعد.</p> : requisitions.map((request) => { const line = request.items?.[0]; const material = inventoryItems.find((item) => item.id === line?.inventoryItemId); const cost = costItems.find((item) => item.id === line?.costItemId); const stage = stages.find((item) => item.id === request.stageId); const pendingApproval = approvals.find((approval) => approval.entityType === "materialRequisition" && approval.entityId === request.id && approval.status === "pending"); const ownerSelection = ownerCostSelections[request.id] || ""; const ownerCostOptions = costItems.filter((item) => item.isActive && item.category === "materials" && (!item.projectId || item.projectId === request.projectId)); const status = line?.planningStatus || "unplanned"; return <div key={request.id} className="rounded-xl border border-slate-100 bg-white p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-bold text-[#18324b]">{request.requestNumber} — {material?.name || line?.description || "خامة غير محددة"}</p><p className="mt-1 text-xs text-slate-500">{projects.find((project) => project.id === request.projectId)?.name} · {stage?.name || "مرحلة غير محددة"} · {cost ? `${cost.code} — ${cost.name}` : "بانتظار تعيين المالك لبند التكلفة"}</p></div><Badge className={planningClass[status as keyof typeof planningClass]}>{planningLabel[status as keyof typeof planningLabel]}</Badge></div><div className="mt-3 grid gap-2 text-xs text-slate-600 sm:grid-cols-4"><span>الكمية: <b>{fmt.format(Number(line?.quantity || 0))} {line?.unit || ""}</b></span><span>المخطط: <b>{fmt.format(Number(line?.plannedQuantity || 0))} {line?.unit || ""}</b></span><span>حالة الطلب: <b>{request.status}</b></span><span>موعد الحاجة: <b>{request.requiredBy ? String(request.requiredBy).slice(0, 10) : "—"}</b></span></div>{user?.role === "admin" && pendingApproval?.approvalStage === "owner" && <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3"><Label className="text-sm font-semibold text-amber-900">تعيين بند التكلفة قبل الاعتماد</Label><div className="mt-2 flex flex-wrap gap-2"><select className="h-10 min-w-64 rounded-md border border-amber-200 bg-white px-3 text-sm" value={ownerSelection} onChange={(event) => setOwnerCostSelections((current) => ({ ...current, [request.id]: event.target.value }))}><option value="">اختر بند التكلفة</option>{ownerCostOptions.map((item) => <option key={item.id} value={item.id}>{item.code} — {item.name}</option>)}</select><Button size="sm" disabled={decide.isPending || !ownerSelection} onClick={() => decide.mutate({ id: request.id, decision: "approved", costItemId: Number(ownerSelection) })} className="bg-emerald-600 hover:bg-emerald-700">تعيين واعتماد</Button><Button size="sm" variant="outline" disabled={decide.isPending} className="border-rose-200 text-rose-700" onClick={() => { const note = window.prompt("اكتب سبب رفض طلب المواد:", ""); if (note?.trim()) decide.mutate({ id: request.id, decision: "rejected", note: note.trim() }); }}>رفض</Button></div><p className="mt-2 text-xs text-amber-800">بعد الاعتماد ينتقل الطلب إلى مدير المشاريع للاعتماد النهائي.</p></div>}<div className="mt-3 flex gap-2"><Button size="sm" variant="outline" onClick={() => edit(request)}><FilePenLine className="ml-1 h-3.5 w-3.5" /> تعديل</Button>{user?.role === "admin" && <Button size="sm" variant="outline" className="border-rose-200 text-rose-700" disabled={remove.isPending} onClick={() => { if (window.confirm("سيُلغى طلب المواد وتُغلق موافقاته المعلقة. هل تريد المتابعة؟")) remove.mutate({ id: request.id }); }}><Trash2 className="ml-1 h-3.5 w-3.5" /> حذف</Button>}</div></div>; })}</div></CardContent></Card>
  </div>;
}
