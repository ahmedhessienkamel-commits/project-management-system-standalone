...
import { AlertTriangle, ClipboardList, FilePenLine, PackageSearch, Plus, Trash2 } from "lucide-react";
...
export function MaterialRequestWorkspace() {
  ...
  const matchingContractLine = useMemo(() => contracts.filter((contract) => contract.projectId === selectedProjectId && contract.status === "active" && (!draft.stageId || !contract.stageId || String(contract.stageId) === draft.stageId)).flatMap((contract) => (contract.contractItems || []).map((line, index) => ({ contract, line, index }))).find(({ line }) => Number(line.inventoryItemId || 0) === selectedItemId), [contracts, selectedProjectId, draft.stageId, selectedItemId]);
  ...
  const create = trpc.erp.procurement.requisitions.create.useMutation({ onSuccess: () => { void utils.erp.procurement.requisitions.list.invalidate(); void utils.erp.approvals.list.invalidate(); setDraft(emptyDraft); } });
  const update = trpc.erp.procurement.requisitions.update.useMutation({ onSuccess: () => { void utils.erp.procurement.requisitions.list.invalidate(); void utils.erp.approvals.list.invalidate(); setEditingId(null); setDraft(emptyDraft); } });
  ...
  useEffect(() => { if (planning.data?.costItemId && !draft.costItemId) setDraft((current) => ({ ...current, costItemId: String(planning.data!.costItemId) })); }, [planning.data?.costItemId, draft.costItemId]);

  const projectStages = stages.filter((stage) => stage.projectId === selectedProjectId);
  const materials = inventoryItems.filter((item) => !item.projectId || item.projectId === selectedProjectId).filter((item) => item.isActive);
  const availableCostItems = costItems.filter((item) => !item.projectId || item.projectId === selectedProjectId).filter((item) => item.isActive && item.category === "materials");
  ...
  return <div className="space-y-6" dir="rtl">
    <header className="rounded-2xl bg-[#18324b] p-6 text-white"><p className="text-sm text-[#f5d98c]">مراقبة طلبات المواد</p><h2 className="mt-1 text-2xl font-bold">طلب مادة مربوط بالخطة والمرحلة</h2><p className="mt-2 max-w-3xl text-sm text-white/75">اختر بطاقة الخامة أولًا، ثم المرحلة وبند التكلفة. يعرض النظام الكمية المتعاقد عليها والطلبات السابقة قبل الإرسال للاعتماد.</p></header>
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,.65fr)]">
      <Card className="border-0 shadow-sm"><CardHeader><CardTitle className="flex items-center gap-2 text-[#18324b]"><Plus className="h-5 w-5" /> {editingId ? "تعديل طلب مادة" : "طلب مادة جديد"}</CardTitle></CardHeader><CardContent><form className="grid gap-4 md:grid-cols-2" onSubmit={save}>
        <Field label="المشروع"><select required className="mt-1 h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm" value={draft.projectId} onChange={(event) => setDraft({ ...draft, projectId: event.target.value, stageId: "", inventoryItemId: "", costItemId: "" })}><option value="">اختر المشروع</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select></Field>
        ...
        <Field label="نوع الخامة / بطاقة المادة"><select required disabled={!selectedProjectId} className="mt-1 h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm" value={draft.inventoryItemId} onChange={(event) => { const item = materials.find((material) => material.id === Number(event.target.value)); const linkedCost = contracts.filter((contract) => contract.projectId === selectedProjectId).flatMap((contract) => contract.contractItems || []).find((line) => Number(line.inventoryItemId || 0) === item?.id)?.costItemId; setDraft({ ...draft, inventoryItemId: event.target.value, costItemId: linkedCost ? String(linkedCost) : "" }); }}><option value="">اختر خامة مثل حديد أو أسمنت</option>{materials.map((item) => <option key={item.id} value={item.id}>{item.code} — {item.name} ({item.unit})</option>)}</select></Field>
        <Field label="بند التكلفة"><select required disabled={!selectedItemId} className="mt-1 h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm" value={draft.costItemId} onChange={(event) => setDraft({ ...draft, costItemId: event.target.value })}><option value="">اختر بند التكلفة</option>{availableCostItems.map((item) => <option key={item.id} value={item.id}>{item.code} — {item.name}</option>)}</select></Field>
        ...
        <div className="md:col-span-2 flex flex-wrap gap-2"><Button className="bg-[#18324b]" disabled={create.isPending || update.isPending || !planningInput.costItemId}>{editingId ? "حفظ وإعادة إرسال الطلب" : "إرسال طلب المواد للاعتماد"}</Button>{editingId && <Button type="button" variant="outline" onClick={() => { setEditingId(null); setDraft(emptyDraft); }}>إلغاء التعديل</Button>}</div>
