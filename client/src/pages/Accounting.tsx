...
const reportTypes = [
  ...
  { key: "costItems", label: "تقرير بنود التكلفة" },
] as const;
...
export default function Accounting() {
  ...
  const [newCostItem, setNewCostItem] = useState({ code: "", name: "", category: "materials" });
  ...
  const createDocument = trpc.erp.accounting.documents.create.useMutation({ onSuccess: () => { utils.erp.accounting.documents.list.invalidate(); setPartyName(""); setAmount(""); setTaxAmount("0"); setNotes(""); setCostItemId(""); } });
  const createCostItem = trpc.erp.costItems.create.useMutation({ onSuccess: () => { utils.erp.costItems.list.invalidate(); setNewCostItem({ code: "", name: "", category: "materials" }); } });
  const reportRange = useMemo(() => ({ from: reportFrom || undefined, to: reportTo || undefined }), [reportFrom, reportTo]);
