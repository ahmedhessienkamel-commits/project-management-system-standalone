...
export default function CompanySettings() {
  ...
  return <DashboardLayout><div dir="rtl" className="min-h-screen bg-[#f7f8fa] px-4 py-6 sm:px-8 lg:px-10"><div className="mx-auto max-w-7xl space-y-6">
    ...
    <div id="cash" className="grid gap-6 lg:grid-cols-[0.85fr_1.15fr]"><Card className="border-0 shadow-sm"><CardHeader><CardTitle className="flex items-center gap-2 text-lg text-[#18324b]"><WalletCards className="h-5 w-5 text-[#b28a3b]" /> إضافة بنك أو خزينة</CardTitle></CardHeader><CardContent><form className="space-y-4" onSubmit={(event) => { event.preventDefault(); if (!cashForm.code.trim() || !cashForm.name.trim()) return; createCash.mutate({ ...cashForm, accountId: cashForm.accountId ? Number(cashForm.accountId) : undefined, openingBalance: Number(cashForm.openingBalance || 0) }); }}><div className="grid gap-3 sm:grid-cols-2"><Field label="كود الحساب *" value={cashForm.code} onChange={(value) => updateCash("code", value)} placeholder="BANK-001 أو CASH-001" /><Field label="اسم الحساب *" value={cashForm.name} onChange={(value) => updateCash("name", value)} placeholder="البنك الرئيسي أو خزينة الموقع" /></div><div className="grid gap-3 sm:grid-cols-2"><div className="space-y-1.5"><Label className="text-sm text-slate-600">نوع الحساب</Label><select className="h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm" value={cashForm.accountType} onChange={(event) => updateCash("accountType", event.target.value as "bank" | "cash")}><option value="cash">خزينة / نقدية</option><option value="bank">حساب بنكي</option></select></div><Field label="العملة" value={cashForm.currency} onChange={(value) => updateCash("currency", value)} /></div><div className="grid gap-3 sm:grid-cols-2"><Field label="اسم البنك" value={cashForm.bankName} onChange={(value) => updateCash("bankName", value)} /><Field label="رقم الحساب" value={cashForm.accountNumber} onChange={(value) => updateCash("accountNumber", value)} /><Field label="IBAN" value={cashForm.iban} onChange={(value) => updateCash("iban", value)} /><Field label="الرصيد الافتتاحي" type="number" value={cashForm.openingBalance} onChange={(value) => updateCash("openingBalance", value)} /></div><div className="space-y-1.5"><Label className="text-sm text-slate-600">الحساب المحاسبي المرتبط</Label><select className="h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm" value={cashForm.accountId} onChange={(event) => updateCash("accountId", event.target.value)}><option value="">اختر حسابًا من الشجرة</option>{accounts.filter((account) => account.accountType === "asset" && account.isPostable).map((account) => <option key={account.id} value={account.id}>{account.code} — {account.name}</option>)}</select></div><Button className="w-full gap-2 bg-[#18324b]" disabled={createCash.isPending}><Landmark className="h-4 w-4" /> حفظ الحساب</Button></form></CardContent></Card><Card className="border-0 shadow-sm"><CardHeader><CardTitle className="text-lg text-[#18324b]">الحسابات البنكية والخزائن</CardTitle></CardHeader><CardContent><div className="space-y-3">{cashAccounts.length ? cashAccounts.map((account) => <div key={account.id} className="flex items-center justify-between gap-3 rounded-xl border border-slate-100 bg-slate-50 p-4"><div><p className="font-bold text-[#18324b]">{account.name} <span className="text-xs font-normal text-slate-500">({account.code})</span></p><p className="mt-1 text-xs text-slate-500">{account.accountType === "bank" ? `بنك ${account.bankName || ""}` : "خزينة / نقدية"} · {account.iban || account.accountNumber || "لا توجد بيانات مصرفية"}</p><p className="mt-1 text-xs text-slate-500">الرصيد الافتتاحي: {account.openingBalance} {account.currency}</p></div><Button size="sm" variant="outline" className="text-rose-700" onClick={() => deactivateCash.mutate({ id: account.id })}>تعطيل</Button></div>) : <p className="py-8 text-center text-sm text-slate-500">لم تتم إضافة حسابات بنكية أو خزائن بعد.</p>}</div></CardContent></Card></div>
