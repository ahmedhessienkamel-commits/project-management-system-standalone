...
function ExecutionBoard({ summary, rows }: { summary: { project: { name: string }; plannedBudget: number; actualCost: number; projectExpensesPreTax?: number; projectExpensesWithTax?: number; administrativeExpensesPreTax?: number; administrativeExpensesTotal: number; totalExpenses: number; cashGap: number; status: string; budgetUsage: number }; rows: Array<{ id: number; code: string; name: string; plannedBudget: number; actual: number; variance: number; consumptionPct: number; timeVarianceDays: number; status: string; actualProgress: number }> }) {
  ...
  const deviation = Number(summary.plannedBudget) - Number(summary.actualCost);
  const priority = (row: typeof rows[number]) => row.timeVarianceDays > 0 || row.variance < 0 ? "عالية" : row.actualProgress < 100 && row.status !== "completed" ? "متابعة" : "عادية";
  const note = (row: typeof rows[number]) => row.timeVarianceDays > 0 ? `متأخرة ${row.timeVarianceDays} يوم` : row.variance < 0 ? "تجاوز في تكلفة المرحلة" : row.status === "completed" ? "مكتملة" : "ضمن الخطة";
  return <Card className="overflow-hidden border-0 shadow-sm"><CardHeader className="bg-[#18324b] text-white"><div className="flex flex-wrap items-center justify-between gap-3"><div><CardTitle className="text-lg">لوحة المؤشرات التنفيذية</CardTitle><p className="mt-1 text-xs text-slate-300">ملخص المشروع المختار ومراقبة مراحل البناء</p></div><Badge className="bg-white/15 text-white">{summary.project.name}</Badge></div></CardHeader><CardContent className="space-y-4 p-0"><div className="grid gap-px bg-slate-200 sm:grid-cols-2 lg:grid-cols-4"><ExecutionMetric label="الميزانية المخططة" value={money.format(summary.plannedBudget)} /><ExecutionMetric label="تكلفة المشروع الفعلية" value={money.format(projectCostPreTax)} /><ExecutionMetric label="المصروفات الإدارية العامة" value={money.format(adminPreTax)} /><ExecutionMetric label="انحراف التكلفة" value={money.format(deviation)} tone={deviation < 0 ? "danger" : "good"} /></div><div className="grid gap-px bg-slate-200 sm:grid-cols-2 lg:grid-cols-4"><ExecutionMetric label="نسبة استهلاك الميزانية" value={`${summary.budgetUsage}%`} /><ExecutionMetric label="المصروفات شامل الضريبة" value={money.format(expensesWithTax)} /><ExecutionMetric label="التمويل المطلوب" value={money.format(summary.cashGap)} tone={summary.cashGap > 0 ? "warn" : "good"} /><ExecutionMetric label="صحة المشروع" value={summary.status === "on_track" ? "مستقرة" : summary.status === "critical" ? "حرجة" : "تحتاج متابعة"} tone={summary.status === "critical" ? "danger" : summary.status === "warning" ? "warn" : "good"} /></div><div className="overflow-x-auto"><table className="w-full min-w-[1050px] text-right text-xs"><thead><tr className="bg-[#2f78b7] text-white"><th className="px-3 py-3">المرحلة</th><th className="px-3 py-3">المخطط</th><th className="px-3 py-3">الفعلي</th><th className="px-3 py-3">الانحراف</th><th className="px-3 py-3">% الانحراف</th><th className="px-3 py-3">الحالة</th><th className="px-3 py-3">أولوية المتابعة</th><th className="px-3 py-3">ملاحظة</th></tr></thead><tbody>{rows.length ? rows.map((row) => { const variancePct = row.plannedBudget ? (Math.abs(row.variance) / row.plannedBudget) * 100 : 0; return <tr key={row.id} className="border-b border-slate-100 odd:bg-slate-50"><td className="px-3 py-3 font-semibold text-[#18324b]">{row.name}</td><td className="px-3 py-3">{money.format(row.plannedBudget)}</td><td className="px-3 py-3">{money.format(row.actual)}</td><td className={`px-3 py-3 font-semibold ${row.variance < 0 ? "text-rose-700" : "text-emerald-700"}`}>{money.format(row.variance)}</td><td className="px-3 py-3">{variancePct.toFixed(1)}%</td><td className="px-3 py-3">{row.status === "completed" ? "مكتملة" : row.status === "delayed" ? "متأخرة" : row.status === "active" ? "مستمرة" : "مخططة"}</td><td className="px-3 py-3">{priority(row)}</td><td className="px-3 py-3 text-slate-500">{note(row)}</td></tr>; }) : <tr><td colSpan={8} className="px-3 py-8 text-center text-slate-500">لا توجد مراحل بناء لهذا المشروع بعد.</td></tr>}</tbody></table></div></CardContent></Card>;
}
