...
export default function Home() {
  ...
  return (
    <DashboardLayout>
      <div dir="rtl" className="min-h-screen bg-[radial-gradient(circle_at_top_right,_#eef5fb_0,_#f7f8fa_42%,_#f7f8fa_100%)] px-4 py-6 sm:px-8 lg:px-10">
        <div className="mx-auto max-w-7xl space-y-6">
          <header className="relative overflow-visible rounded-[2rem] border border-white/80 bg-white/90 p-5 shadow-[0_18px_60px_-32px_rgba(24,50,75,0.45)] backdrop-blur sm:p-7 lg:flex lg:flex-row lg:items-end lg:justify-between">
            <div className="relative z-40 order-2 w-full rounded-2xl border border-[#d9e4ee] bg-[#f8fbfd] p-3 shadow-inner lg:order-1 lg:w-80"><div className="mb-2 flex items-center justify-between"><label className="text-xs font-bold text-[#18324b]">المشروع المعروض في المؤشرات</label><span className="rounded-full bg-[#e8f1f8] px-2 py-1 text-[10px] font-semibold text-[#2f78b7]">تصفية فورية</span></div><select aria-label="اختيار المشروع المعروض في المؤشرات" value={selectedSummary ? String(selectedSummary.project.id) : ""} onChange={(event) => setSelectedProjectId(Number(event.target.value))} className="h-12 w-full cursor-pointer rounded-xl border border-slate-200 bg-white px-3 text-sm font-bold text-[#18324b] shadow-sm outline-none transition hover:border-[#b28a3b] focus:border-[#b28a3b] focus:ring-4 focus:ring-[#b28a3b]/10"><option value="" disabled>اختر مشروعًا</option>{summaries.map((item) => <option key={item.project.id} value={item.project.id}>{item.project.name}</option>)}</select><p className="mt-2 text-[11px] text-slate-500">تتحدث المؤشرات والسيولة والمراحل فورًا عند التغيير</p></div>
            <div className="order-1 lg:order-2">
              <p className="mb-2 inline-flex rounded-full bg-[#f5f0e5] px-3 py-1 text-xs font-bold tracking-wide text-[#9a7427]">مركز القيادة التنفيذية</p>
              <h1 className="text-3xl font-black tracking-tight text-[#18324b] sm:text-4xl lg:text-5xl">صورة المشروع في لحظة</h1>
...
function ExecutionBoard({ summary, rows }: { summary: { project: { name: string }; plannedBudget: number; actualCost: number; projectExpensesPreTax?: number; projectExpensesWithTax?: number; administrativeExpensesPreTax?: number; administrativeExpensesTotal: number; totalExpenses: number; cashGap: number; status: string; budgetUsage: number; projectTimeVarianceDays?: number; projectTimeStatus?: string; stageCount?: number; activeStage?: { id: number; name: string; plannedBudget: number; actualCost: number; actualProgress: number } | null }; rows: Array<{ id: number; code: string; name: string; plannedBudget: number; actual: number; variance: number; consumptionPct: number; timeVarianceDays: number; status: string; actualProgress: number }> }) {
  ...
  const stageContributionPct = summary.stageCount ? ((activeStageRow?.actualProgress ?? 0) / summary.stageCount) : 0;
