    ...
    <DashboardLayout>
      <div dir="rtl" className="min-h-screen bg-[#f7f8fa] px-4 py-6 sm:px-8 lg:px-10">
        <div className="mx-auto max-w-7xl space-y-8">
          ...
          {selectedSummary && <DashboardStageDetail rows={selectedDetailReport?.rows ?? []} total={selectedDetailReport?.total ?? null} onOpenReport={() => setLocation("/projects")} />}
...
function DashboardStageDetail({ rows, total, onOpenReport }: { rows: Array<{ rowType: "stage" | "costItem"; id: number; code: string; name: string; stageName: string; plannedBudget: number; actual: number; paidAmount: number; outstanding: number; variance: number; consumptionPct: number; timeVarianceDays: number; status: string }>; total: { plannedBudget: number; actual: number; paidAmount: number; outstanding: number; variance: number; consumptionPct: number } | null; onOpenReport: () => void }) {
  return <Card className="border-0 shadow-sm"><CardHeader className="flex flex-row items-center justify-between"><div><CardTitle className="text-lg text-[#18324b]">تفصيل أداء وتكلفة المراحل</CardTitle><p className="mt-1 text-xs text-slate-500">نفس ملخص التقرير التفصيلي ظاهر مباشرة في لوحة التنفيذ للمشروع المختار.</p></div><Button variant="outline" size="sm" onClick={onOpenReport}>فتح تفصيل المشاريع</Button></CardHeader><CardContent className="overflow-x-auto"><table className="w-full min-w-[1000px] text-right text-xs"><thead><tr className="bg-[#18324b] text-white"><th className="px-3 py-3">الكود</th><th className="px-3 py-3">المرحلة / البند</th><th className="px-3 py-3">المخطط</th><th className="px-3 py-3">الفعلي</th><th className="px-3 py-3">المدفوع</th><th className="px-3 py-3">المستحق</th><th className="px-3 py-3">الانحراف</th><th className="px-3 py-3">الاستهلاك</th><th className="px-3 py-3">انحراف الوقت</th><th className="px-3 py-3">الحالة</th></tr></thead><tbody>{rows.length ? rows.map((row) => <tr key={`${row.rowType}-${row.id}`} className="border-b border-slate-100 odd:bg-slate-50"><td className="px-3 py-3 font-semibold text-[#18324b]">{row.code}</td><td className="px-3 py-3"><b>{row.name}</b>{row.rowType === "costItem" && <small className="block text-slate-400">ضمن {row.stageName}</small>}</td><td className="px-3 py-3">{money.format(row.plannedBudget)}</td><td className="px-3 py-3">{money.format(row.actual)}</td><td className="px-3 py-3 text-emerald-700">{money.format(row.paidAmount)}</td><td className="px-3 py-3 text-amber-700">{money.format(row.outstanding)}</td><td className={`px-3 py-3 ${row.variance < 0 ? "text-rose-700" : "text-emerald-700"}`}>{money.format(row.variance)}</td><td className="px-3 py-3">{row.plannedBudget ? `${row.consumptionPct.toFixed(1)}%` : "—"}</td><td className={`px-3 py-3 ${row.timeVarianceDays > 0 ? "text-rose-700" : "text-emerald-700"}`}>{row.timeVarianceDays > 0 ? `${row.timeVarianceDays} يوم` : "لا يوجد"}</td><td className="px-3 py-3">{row.status === "completed" ? "مكتملة" : row.status === "active" ? "مستمرة" : row.status === "delayed" ? "مستمرة ومتأخرة" : "مخططة"}</td></tr>) : <tr><td colSpan={10} className="py-8 text-center text-slate-500">لا توجد مراحل أو بنود تكلفة مسجلة لهذا المشروع.</td></tr>}</tbody><tfoot><tr className="bg-[#f5f0e5] font-bold text-[#18324b]"><td className="px-3 py-3" colSpan={2}>الإجمالي</td><td className="px-3 py-3">{money.format(total?.plannedBudget ?? 0)}</td><td className="px-3 py-3">{money.format(total?.actual ?? 0)}</td><td className="px-3 py-3">{money.format(total?.paidAmount ?? 0)}</td><td className="px-3 py-3">{money.format(total?.outstanding ?? 0)}</td><td className="px-3 py-3">{money.format(total?.variance ?? 0)}</td><td className="px-3 py-3">{total?.plannedBudget ? `${total.consumptionPct.toFixed(1)}%` : "—"}</td><td className="px-3 py-3">—</td><td /></tr></tfoot></table></CardContent></Card>;
}
