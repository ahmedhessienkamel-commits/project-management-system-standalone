...
type SheetPreview = { name: string; rows: number; columns: number; duplicateRows: number; headers: string[]; sample: string[][] };
...
export default function LegacyImport() {
  ...
  const readFile = async (file: File) => {
    ...
    const previews = workbook.SheetNames.map((name) => {
      ...
      const keys = dataRows.map((row) => row.map((value) => String(value).trim()).join("¦"));
      const seen = new Set<string>();
      const duplicateRows = keys.filter((key) => seen.has(key) || (seen.add(key), false)).length;
      const headers = (nonEmpty[0] ?? []).slice(0, 12).map((value) => String(value));
      const sample = dataRows.slice(0, 4).map((row) => row.slice(0, 12).map((value) => String(value)));
      return { name, rows: dataRows.length, columns: headers.length, duplicateRows, headers, sample };
    });
    setSheets(previews);
