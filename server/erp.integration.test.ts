...
describe("ERP sales and collections API flow", () => {
  ...
  it("creates a confirmed unit sale, received collection, and dashboard summary from the same state", async () => {
    ...
    const certificate = await caller.erp.certificates.create({ projectId: 1, certificateNumber: "CERT-001", description: "مستخلص اختبار", preTaxAmount: 5000, taxRate: 15, paidAmount: 1000 });
    ...
    const custody = await caller.erp.custody.create({ projectId: 1, holderName: "أحمد", issuedAmount: 2000, settledAmount: 500 });
