...
export function calculateSupplierStatementTotals({ vendorId, expenses, certificates, invoices, vouchers }: { vendorId: number; expenses: Array<{ id: number; vendorId?: number | null; expenseDate?: Date | string | null; reference?: string | null; description?: string | null; totalAmount?: string | number | null; paidAmount?: string | number | null }>; certificates: Array<{ id: number; vendorId?: number | null; certificateDate?: Date | string | null; certificateNumber?: string | null; description?: string | null; totalAmount?: string | number | null; paidAmount?: string | number | null }>; invoices: Array<{ id: number; supplierId?: number | null; contractorId?: number | null; certificateId?: number | null; totalAmount?: string | number | null; paidAmount?: string | number | null; relatedDocumentType?: string | null; relatedDocumentId?: number | null }>; vouchers: Array<{ id: number; supplierId?: number | null; contractorId?: number | null; certificateId?: number | null; purchaseInvoiceId?: number | null; documentDate?: Date | string | null; documentNumber?: string | null; notes?: string | null; totalAmount?: string | number | null }> }) {
  ...
  const invoiceCertificateIds = new Map(invoices.filter((row) => row.certificateId && certificateIds.has(row.certificateId)).map((row) => [row.id, row.certificateId!]));
  const vendorInvoices = invoices.filter((row) => (row.supplierId === vendorId || row.contractorId === vendorId) && !(row.certificateId && certificateIds.has(row.certificateId)) && !(row.relatedDocumentType === "certificate" && row.relatedDocumentId && certificateIds.has(row.relatedDocumentId)));
  const invoiceIds = new Set(vendorInvoices.map((row) => row.id));
  const vendorVouchers = vouchers.filter((row) => row.supplierId === vendorId || row.contractorId === vendorId || (row.purchaseInvoiceId ? invoiceIds.has(row.purchaseInvoiceId) || invoiceCertificateIds.has(row.purchaseInvoiceId) : false) || (row.certificateId ? certificateIds.has(row.certificateId) : false));
  ...
  const certificatePaid = vendorCertificates.reduce((sum, row) => sum + Math.max(Number(row.paidAmount || 0), vendorVouchers.filter((voucher) => voucher.certificateId === row.id || (voucher.purchaseInvoiceId && invoiceCertificateIds.get(voucher.purchaseInvoiceId) === row.id)).reduce((value, voucher) => value + Number(voucher.totalAmount || 0), 0)), 0);
  ...
  const directVoucherPaid = vendorVouchers.filter((voucher) => !voucher.certificateId && !(voucher.purchaseInvoiceId && (invoiceIds.has(voucher.purchaseInvoiceId) || invoiceCertificateIds.has(voucher.purchaseInvoiceId)))).reduce((sum, voucher) => sum + Number(voucher.totalAmount || 0), 0);
