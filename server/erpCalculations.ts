...
export function projectHealthStatus({ budgetUsage, progress, delayedStages, cashGapRatio = 0, pendingApprovals = 0 }: { budgetUsage: number; progress: number; delayedStages: number; cashGapRatio?: number; pendingApprovals?: number }) {
  if (delayedStages > 0 || budgetUsage >= 100 || cashGapRatio >= 0.5 || pendingApprovals >= 3) return "critical" as const;
  if (budgetUsage >= 80 || progress < 20 || cashGapRatio > 0 || pendingApprovals > 0) return "warning" as const;
  return "on_track" as const;
}

export function projectHealthReasons({ budgetUsage, progress, delayedStages, cashGap, pendingApprovals }: { budgetUsage: number; progress: number; delayedStages: number; cashGap: number; pendingApprovals: number }) {
