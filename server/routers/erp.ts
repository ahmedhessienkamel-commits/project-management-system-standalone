...
import { getAssetPaymentVoucherValidationMessage } from "../../shared/paymentVoucher";
...
export const erpRouter = router({
  ...
  leaveRequests: router({
    list: protectedProcedure.query(async ({ ctx }) => {
      const db = requireDb(await getDb());
      const [rows, employeeRows, userRows] = await Promise.all([
        db.select().from(leaveRequests).orderBy(desc(leaveRequests.createdAt)),
        db.select().from(employees),
        db.select().from(users),
      ]);
      const canReviewAll = ctx.user.role === "admin" || ctx.user.role === "general_manager" || Number(ctx.user.id) === 13170001;
  ...
  advanceRequests: router({
    list: protectedProcedure.query(async ({ ctx }) => {
      const db = requireDb(await getDb());
      const [rows, repayments, employeeRows, userRows] = await Promise.all([
        db.select().from(advanceRequests).orderBy(desc(advanceRequests.createdAt)),
        db.select().from(advanceRepayments),
        db.select().from(employees),
        db.select().from(users),
      ]);
      const canReviewAll = ctx.user.role === "admin" || ctx.user.role === "general_manager" || Number(ctx.user.id) === 13170001;
      const visible = canReviewAll ? rows : rows.filter((row) => row.requestedBy === ctx.user.id);
  ...
  projects: router({
    ...
    auditByName: adminProcedure.input(z.object({ name: z.string().trim().min(2).max(255) })).query(async ({ input }) => {
      const db = requireDb(await getDb());
      const normalized = input.name.toLocaleLowerCase("ar-SA");
      const matches = (await db.select().from(projects)).filter((project) => project.name.toLocaleLowerCase("ar-SA").includes(normalized));
      return Promise.all(matches.map(async (project) => {
        const [stageRows, expenseRows, payrollRows, custodyRows, certificateRows, contractRows, salesRows, collectionRows, memberRows, approvalRows, attachmentRows, requisitionRows, orderRows, receiptRows, movementRows, documentRows, lineRows] = await Promise.all([
          db.select().from(stages).where(eq(stages.projectId, project.id)), db.select().from(expenses).where(eq(expenses.projectId, project.id)), db.select().from(payroll).where(eq(payroll.projectId, project.id)), db.select().from(custody).where(eq(custody.projectId, project.id)), db.select().from(certificates).where(eq(certificates.projectId, project.id)), db.select().from(contractorContracts).where(eq(contractorContracts.projectId, project.id)), db.select().from(sales).where(eq(sales.projectId, project.id)), db.select().from(collections).where(eq(collections.projectId, project.id)), db.select().from(projectMembers).where(eq(projectMembers.projectId, project.id)), db.select().from(approvalRequests).where(eq(approvalRequests.projectId, project.id)), db.select().from(attachments).where(eq(attachments.projectId, project.id)), db.select().from(materialRequisitions).where(eq(materialRequisitions.projectId, project.id)), db.select().from(purchaseOrders).where(eq(purchaseOrders.projectId, project.id)), db.select().from(purchaseReceipts).where(eq(purchaseReceipts.projectId, project.id)), db.select().from(inventoryMovements).where(eq(inventoryMovements.projectId, project.id)), db.select().from(accountingDocuments).where(eq(accountingDocuments.projectId, project.id)), db.select().from(accountingDocumentLines).where(eq(accountingDocumentLines.projectId, project.id)),
        ]);
    ...
    deleteConfirmedEmpty: adminProcedure.input(z.object({ code: z.literal("002"), name: z.literal("المهدية"), confirmed: z.literal(true) })).mutation(async ({ ctx, input }) => {
      const db = requireDb(await getDb());
      const project = (await db.select().from(projects).where(and(eq(projects.code, input.code), eq(projects.name, input.name))).limit(1))[0];
      if (!project) throw new TRPCError({ code: "NOT_FOUND", message: "لم يعد سجل المهدية موجودًا" });
      const [stageRows, expenseRows, payrollRows, custodyRows, certificateRows, contractRows, salesRows, collectionRows, memberRows, approvalRows, attachmentRows, requisitionRows, orderRows, receiptRows, movementRows, documentRows, lineRows] = await Promise.all([
        db.select({ id: stages.id }).from(stages).where(eq(stages.projectId, project.id)), db.select({ id: expenses.id }).from(expenses).where(eq(expenses.projectId, project.id)), db.select({ id: payroll.id }).from(payroll).where(eq(payroll.projectId, project.id)), db.select({ id: custody.id }).from(custody).where(eq(custody.projectId, project.id)), db.select({ id: certificates.id }).from(certificates).where(eq(certificates.projectId, project.id)), db.select({ id: contractorContracts.id }).from(contractorContracts).where(eq(contractorContracts.projectId, project.id)), db.select({ id: sales.id }).from(sales).where(eq(sales.projectId, project.id)), db.select({ id: collections.id }).from(collections).where(eq(collections.projectId, project.id)), db.select({ id: projectMembers.id }).from(projectMembers).where(eq(projectMembers.projectId, project.id)), db.select({ id: approvalRequests.id }).from(approvalRequests).where(eq(approvalRequests.projectId, project.id)), db.select({ id: attachments.id }).from(attachments).where(eq(attachments.projectId, project.id)), db.select({ id: materialRequisitions.id }).from(materialRequisitions).where(eq(materialRequisitions.projectId, project.id)), db.select({ id: purchaseOrders.id }).from(purchaseOrders).where(eq(purchaseOrders.projectId, project.id)), db.select({ id: purchaseReceipts.id }).from(purchaseReceipts).where(eq(purchaseReceipts.projectId, project.id)), db.select({ id: inventoryMovements.id }).from(inventoryMovements).where(eq(inventoryMovements.projectId, project.id)), db.select({ id: accountingDocuments.id }).from(accountingDocuments).where(eq(accountingDocuments.projectId, project.id)), db.select({ id: accountingDocumentLines.id }).from(accountingDocumentLines).where(eq(accountingDocumentLines.projectId, project.id)),
      ]);
      const relatedCount = [stageRows, expenseRows, payrollRows, custodyRows, certificateRows, contractRows, salesRows, collectionRows, memberRows, approvalRows, attachmentRows, requisitionRows, orderRows, receiptRows, movementRows, documentRows, lineRows].reduce((count, rows) => count + rows.length, 0);
      if (relatedCount > 0) throw new TRPCError({ code: "CONFLICT", message: "توقف الحذف: أصبح سجل المهدية مرتبطًا بسجلات أخرى بعد الحصر. لم يُحذف أي شيء." });
      await db.delete(projects).where(eq(projects.id, project.id));
      await db.insert(auditLogs).values({ entityType: "project", entityId: project.id, action: "deleted_confirmed_empty_mahdiya", actorId: ctx.user.id, beforeJson: JSON.stringify(project), afterJson: JSON.stringify({ code: input.code, name: input.name, relatedCount }) });
  ...
  custodyMovements: router({
    ...
    create: protectedProcedure.input(z.object({ projectId: z.number().int().positive().optional(), stageId: z.number().int().positive().optional(), employeeCode: z.string().trim().min(1), employeeName: z.string().trim().min(2), movementType: z.enum(["issue", "spend", "return", "settlement"]), allocationType: z.enum(["project", "general_cash", "general_admin", "petty_cash", "operating_expense"]), description: z.string().trim().min(2), amount: z.number().positive(), movementDate: z.string().optional(), expenseType: z.string().trim().max(64).optional(), vendorId: z.number().int().positive().optional(), payrollBeneficiaryType: z.enum(["company_employee", "worker"]).optional(), payrollEmployeeId: z.number().int().positive().optional(), payrollBeneficiaryName: z.string().trim().max(255).optional(), repaymentMode: z.enum(["single", "installments"]).optional(), repaymentStartMonth: z.number().int().min(1).max(12).optional(), repaymentStartYear: z.number().int().min(2000).max(2100).optional(), installmentCount: z.number().int().min(1).max(120).optional(), allocationRatio: z.number().min(0).max(100).default(100) })).mutation(async ({ ctx, input }) => {
      const db = requireDb(await getDb());
      const companyId = await resolveActiveCompanyId(db, ctx);
      ...
      let advanceBeneficiaryId: number | null = null;
      if (input.expenseType === "advance") {
        if (input.movementType !== "spend") throw new TRPCError({ code: "BAD_REQUEST", message: "السلفة تُسجل كصرف من العهدة فقط" });
        if (!input.payrollBeneficiaryType) throw new TRPCError({ code: "BAD_REQUEST", message: "يجب اختيار الموظف أو الأجير المستفيد من السلفة" });
        const [beneficiary] = input.payrollEmployeeId ? await db.select().from(employees).where(eq(employees.id, input.payrollEmployeeId)).limit(1) : input.payrollBeneficiaryName ? await db.select().from(employees).where(eq(employees.fullName, input.payrollBeneficiaryName)).limit(1) : [];
        if (!beneficiary) throw new TRPCError({ code: "NOT_FOUND", message: "الموظف أو الأجير المستفيد غير موجود" });
        const expectedType = input.payrollBeneficiaryType === "worker" ? "worker" : "employee";
        if (beneficiary.employmentType !== expectedType) throw new TRPCError({ code: "BAD_REQUEST", message: "نوع المستفيد لا يطابق نوع السجل المختار" });
        advanceBeneficiaryId = beneficiary.id;
      }
      const storedAllocationType = input.allocationType;
      const result = await db.insert(custodyMovements).values({ companyId: companyId || null, projectId: input.projectId || null, stageId: input.stageId || null, employeeCode: input.employeeCode, employeeName: input.employeeName, movementType: input.movementType, allocationType: storedAllocationType, description: input.description, amount: input.amount.toFixed(2), signedAmount: signedAmount.toFixed(2), movementDate: input.movementDate ? new Date(input.movementDate) : null, expenseType: input.expenseType || null, vendorId: input.vendorId || null, payrollBeneficiaryType: input.payrollBeneficiaryType || null, payrollEmployeeId: input.payrollEmployeeId || null, payrollBeneficiaryName: input.payrollBeneficiaryName || null, allocationRatio: input.allocationRatio.toFixed(2), createdBy: ctx.user.id });
      const id = Number(result[0].insertId);
      let linkedExpenseId: number | null = null;
      let advanceRequestId: number | null = null;
      if (input.expenseType === "advance") {
        const plan = resolveAdvancePlan(input);
        const advanceResult = await db.insert(advanceRequests).values({ requestedBy: ctx.user.id, employeeId: advanceBeneficiaryId!, amount: input.amount.toFixed(2), reason: `سلفة من العهدة: ${input.description}`, repaymentDate: null, ...plan });
        advanceRequestId = Number(advanceResult[0].insertId);
        await db.insert(auditLogs).values({ entityType: "advanceRequest", entityId: advanceRequestId, action: "created_from_custody", actorId: ctx.user.id, afterJson: JSON.stringify({ custodyMovementId: id, ...input }) });
      } else if (["spend", "settlement"].includes(input.movementType)) {
      ...
      await db.insert(auditLogs).values({ entityType: "custodyMovement", entityId: id, action: "created", actorId: ctx.user.id, afterJson: JSON.stringify({ ...input, signedAmount, linkedExpenseId, advanceRequestId }) });
      return { id, signedAmount, linkedExpenseId, advanceRequestId };
    }),
    ...
    createAdministrativeExpense: protectedProcedure.input(z.object({ employeeCode: z.string().trim().min(1), employeeName: z.string().trim().min(2), description: z.string().trim().min(2), amount: z.number().positive(), costItemId: z.number().int().positive(), movementDate: z.string().optional() })).mutation(async ({ ctx, input }) => {
      const db = requireDb(await getDb());
      await assertOperationPermission(db, ctx, "custody");
      const companyId = await resolveActiveCompanyId(db, ctx);
      const [costItem] = await db.select().from(costItems).where(eq(costItems.id, input.costItemId)).limit(1);
      if (!costItem || costItem.isActive === 0) throw new TRPCError({ code: "BAD_REQUEST", message: "بند المصروف الإداري غير صالح أو غير نشط" });
      const result = await db.insert(custodyMovements).values({ companyId: companyId || null, employeeCode: input.employeeCode, employeeName: input.employeeName, movementType: "spend", allocationType: "general_admin", description: input.description, amount: input.amount.toFixed(2), signedAmount: (-input.amount).toFixed(2), movementDate: input.movementDate ? new Date(input.movementDate) : null, expenseType: "administrative", createdBy: ctx.user.id });
      const id = Number(result[0].insertId);
      const expenseResult = await db.insert(expenses).values({ companyId: companyId || null, projectId: null, stageId: null, costItemId: input.costItemId, description: `مصروف عهدة إداري #${id}: ${input.description}`, unit: "مبلغ", quantity: "1", expenseType: "administrative", classification: "administrative", allocationRatio: "1.000", preTaxAmount: input.amount.toFixed(2), taxRate: "0", taxAmount: "0", totalAmount: input.amount.toFixed(2), paidAmount: input.amount.toFixed(2), status: "approved", expenseDate: input.movementDate ? new Date(input.movementDate) : null, createdBy: ctx.user.id });
      const linkedExpenseId = Number(expenseResult[0].insertId);
      await db.update(custodyMovements).set({ linkedExpenseId }).where(eq(custodyMovements.id, id));
      await db.insert(auditLogs).values({ entityType: "custodyMovement", entityId: id, action: "administrative_expense_created", actorId: ctx.user.id, afterJson: JSON.stringify({ ...input, linkedExpenseId }) });
      return { id, linkedExpenseId };
    }),
  }),
  attendance: router({
    list: protectedProcedure.query(async ({ ctx }) => {
  ...
  accounting: router({
    ...
    documents: router({
      ...
      create: protectedProcedure.input(z.object({ projectId: z.number().int().positive().optional(), documentType: z.enum(["sales_invoice", "purchase_invoice", "purchase_receipt", "credit_note", "journal_entry", "payment_voucher", "receipt_voucher", "quotation", "purchase_order"]), relatedDocumentType: z.enum(["quotation", "certificate"]).optional(), relatedDocumentId: z.number().int().positive().optional(), originalDocumentId: z.number().int().positive().optional(), sourceDocumentId: z.number().int().positive().optional(), returnType: z.enum(["full", "partial"]).optional(), voucherCategory: z.enum(["contractor", "supplier", "materials", "payroll", "operating", "administrative", "petty_cash", "asset"]).optional(), contractorId: z.number().int().positive().optional(), supplierId: z.number().int().positive().optional(), purchaseInvoiceId: z.number().int().positive().optional(), settlementType: z.enum(["invoice", "direct"]).optional(), certificateId: z.number().int().positive().optional(), fixedAssetId: z.number().int().positive().optional(), partyName: z.string().max(255).optional(), partyTaxNumber: z.string().max(64).optional(), documentDate: z.string().optional(), dueDate: z.string().optional(), sourceAccountId: z.number().int().positive().optional(), amount: z.number().nonnegative(), taxAmount: z.number().nonnegative(), totalAmount: z.number().nonnegative(), paymentMethod: z.enum(["cash", "bank"]).optional(), notes: z.string().max(2000).optional(), status: z.enum(["draft", "posted"]).default("draft"), lines: z.array(z.object({ accountId: z.number().int().positive(), costItemId: z.number().int().positive().optional(), projectId: z.number().int().positive().optional(), stageId: z.number().int().positive().optional(), description: z.string().max(500).optional(), debit: z.number().nonnegative(), credit: z.number().nonnegative() })).min(1) })).mutation(async ({ ctx, input }) => {
        ...
        if (input.documentType === "payment_voucher") {
          ...
          if (input.voucherCategory === "asset") {
            if (!input.fixedAssetId) throw new TRPCError({ code: "BAD_REQUEST", message: "اختر بطاقة الأصل المرتبط بسند الصرف" });
            const asset = (await db.select({ id: fixedAssets.id, projectId: fixedAssets.projectId, assetAccountId: fixedAssets.assetAccountId }).from(fixedAssets).where(and(eq(fixedAssets.id, input.fixedAssetId), eq(fixedAssets.status, "active"))).limit(1))[0];
            if (!asset) throw new TRPCError({ code: "NOT_FOUND", message: "بطاقة الأصل غير موجودة أو غير نشطة" });
            if (asset.projectId) {
              await assertProjectAccess(db, ctx, asset.projectId);
              if (input.projectId && input.projectId !== asset.projectId) throw new TRPCError({ code: "BAD_REQUEST", message: "بطاقة الأصل لا تخص المشروع المحدد" });
            }
            const debitLines = input.lines.filter((line) => line.debit > 0);
            const creditLines = input.lines.filter((line) => line.credit > 0);
            const assetValidationMessage = getAssetPaymentVoucherValidationMessage({ hasAssetCard: true, assetAccountId: asset.assetAccountId, debitAccountId: debitLines[0]?.accountId, debitLineCount: debitLines.length, creditLineCount: creditLines.length, hasCostItem: input.lines.some((line) => Boolean(line.costItemId)) });
            if (assetValidationMessage) throw new TRPCError({ code: "BAD_REQUEST", message: assetValidationMessage });
          }
          if (input.voucherCategory === "supplier" || input.voucherCategory === "asset") {
            if (input.voucherCategory === "supplier" && !input.supplierId) throw new TRPCError({ code: "BAD_REQUEST", message: "حدد المورد المرتبط بسند الصرف" });
            ...
            if (input.supplierId) {
              supplier = (await db.select({ id: vendors.id, name: vendors.name }).from(vendors).where(eq(vendors.id, input.supplierId)).limit(1))[0];
            ...
            if (input.purchaseInvoiceId) {
              ...
              if (!purchaseInvoice || purchaseInvoice.documentType !== "purchase_invoice" || (supplier && purchaseInvoice.partyName !== supplier.name)) throw new TRPCError({ code: "BAD_REQUEST", message: "فاتورة الشراء لا تخص المورد المحدد" });
            }
          }
    ...
    reports: router({
      ...
      incomeStatement: protectedProcedure.input(z.object({ projectId: z.number().int().positive().optional(), from: z.string().optional(), to: z.string().optional() }).optional()).query(async ({ input }) => {
        ...
        const [salesRows, expenseRows, payrollRows, certificateRows, voucherRows, projectRows] = await Promise.all([db.select().from(sales), db.select().from(expenses), db.select().from(payroll), db.select().from(certificates), db.select().from(accountingDocuments).where(eq(accountingDocuments.documentType, "payment_voucher")), db.select().from(projects)]);
        const from = input?.from ? new Date(input.from).getTime() : Number.NEGATIVE_INFINITY;
        const to = input?.to ? new Date(input.to).getTime() + 86400000 : Number.POSITIVE_INFINITY;
      ...
      accountStatement: protectedProcedure.input(z.object({ accountId: z.number().int().positive(), projectId: z.number().int().positive().optional(), from: z.string().optional(), to: z.string().optional() })).query(async ({ ctx, input }) => {
        const db = requireDb(await getDb());
        const companyId = await resolveActiveCompanyId(db, ctx);
        if (!companyId) throw new TRPCError({ code: "PRECONDITION_FAILED", message: "لا توجد شركة نشطة" });
        const account = (await db.select().from(accounts).where(and(eq(accounts.id, input.accountId), eq(accounts.companyId, companyId))).limit(1))[0];
        if (!account) throw new TRPCError({ code: "NOT_FOUND", message: "الحساب غير موجود" });
        const rows = sortStatementRowsAscending((await loadAccountingLedger(db, input, companyId)).filter((row) => row.accountId === input.accountId));
        let balance = 0;
  ...
  reports: router({
    ...
    supplierStatement: protectedProcedure.input(z.object({ projectId: z.number().int().positive(), vendorId: z.number().int().positive() })).query(async ({ ctx, input }) => {
      const db = requireDb(await getDb());
      await assertProjectAccess(db, ctx, input.projectId);
      const [expenseRows, certificateRows, invoiceRows, voucherRows] = await Promise.all([
        db.select().from(expenses).where(eq(expenses.projectId, input.projectId)),
        db.select().from(certificates).where(eq(certificates.projectId, input.projectId)),
        ...
        db.select().from(accountingDocuments).where(and(eq(accountingDocuments.projectId, input.projectId), eq(accountingDocuments.documentType, "payment_voucher"), eq(accountingDocuments.status, "posted"))),
      ]);
      return calculateSupplierStatementTotals({ vendorId: input.vendorId, expenses: expenseRows, certificates: certificateRows, invoices: invoiceRows, vouchers: voucherRows });
    }),
