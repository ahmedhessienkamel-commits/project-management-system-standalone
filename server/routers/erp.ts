  ...
  dashboard: router({
    summary: protectedProcedure.query(async ({ ctx }) => {
      ...
      const summary = projectRows.map((project) => {
        ...
        // Whole-project comparison uses the project-level estimate; stage budgets remain stage-only baselines.
        const planned = Number(project.estimatedTotalCost || 0);
