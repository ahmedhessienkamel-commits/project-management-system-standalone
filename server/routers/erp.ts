...
export const erpRouter = router({
  ...
  dashboard: router({
    ...
    companySummary: protectedProcedure.query(async ({ ctx }) => {
      ...
      const legacyAdministrativePayrollRows = payrollRows.filter((row) => row.projectId === null && row.classification === "administrative" && approved(row.status));
      const legacyAdministrativePayroll = legacyAdministrativePayrollRows.reduce((sum, row) => sum + Number(row.totalAmount || 0), 0);
      const legacyAdministrativePayrollPaid = legacyAdministrativePayrollRows.reduce((sum, row) => sum + Number(row.paidAmount || 0), 0);
      const approvedAdministrativePayrollRows = administrativePayrollRows.filter((row: typeof administrativePayrollRows[number]) => ["approved", "paid"].includes(row.status));
      const administrativePayrollTotal = approvedAdministrativePayrollRows.reduce((sum: number, row: typeof administrativePayrollRows[number]) => sum + Number(row.totalAmount || 0), 0) + legacyAdministrativePayroll;
      const administrativePayrollPaid = approvedAdministrativePayrollRows.reduce((sum: number, row: typeof administrativePayrollRows[number]) => sum + Number(row.paidAmount || 0), 0) + legacyAdministrativePayrollPaid;
      const administrativePayrollOutstanding = Math.max(administrativePayrollTotal - administrativePayrollPaid, 0);
      ...
      return { projectCosts: projectExpenses + projectPayroll + projectVoucherCosts + subcontractorCosts + inventoryCosts, projectRevenue, administrativeExpenses, pettyCashExpenses, administrativePayroll: administrativePayrollTotal, administrativePayrollOutstanding, sharedTotal, activeProjects: allocation, allocationBasis: "contract_value" as const };
    }),
  }),
