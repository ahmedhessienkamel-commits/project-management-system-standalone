...
export function calculateMaterialPlanning(input: MaterialPlanningInput) {
  ...
  if (!plannedQuantity) {
    return {
      ...
      plannedQuantity: 0,
      committedQuantity: Math.max(requestedBeforeQuantity, suppliedQuantity),
      remainingQuantity: 0,
      requestedAfterQuantity: requestedQuantity,
      varianceQuantity: requestedQuantity,
      isWithinPlan: false,
    };
  }
  ...
  return {
    status: (varianceQuantity > 0 ? "over_plan" : "within_plan") as MaterialPlanningStatus,
    plannedQuantity,
    committedQuantity,
    remainingQuantity,
    requestedAfterQuantity,
    varianceQuantity,
    isWithinPlan: varianceQuantity === 0,
  };
}
